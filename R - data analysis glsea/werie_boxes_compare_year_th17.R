...
## August 8, 2018
## Updated by Grace Di Cecco, 5/19/23
## Summarize and domain averaging on modeled hab

# Input directory
dirin = 'C:/Users/Grace Di Cecco/OneDrive - Eagle Rock Analytics/Desktop/Postdoc-EagleRock/hab_proj/HAB_code/output_data_glsea'
setwd(dirin)

# Graphics directory
dirout = 'C:/Users/Grace Di Cecco/OneDrive - Eagle Rock Analytics/Desktop/Postdoc-EagleRock/hab_proj/HAB_code/pdf_glsea'
...
nc = nc_open('annual.micro.werie.growsea-th17.nc', write=FALSE, readunlim =FALSE)
...
# Years to compare
cmp_yrs = 5
start_yrs <- year[1:cmp_yrs]
end_yrs <- tail(year,n=cmp_yrs)
...
# 6: Merge by station
#########
mb.all <- merge(first.mb,last.mb,by='year')
library(plyr)
mb.all<-rename(mb.all,c('med.x'='first.med','hi.x'='first.hi','lo.x'='first.lo','med.y'='last.med','hi.y'='last.hi','lo.y'='last.lo'))
mb.all<-merge(mb.all,gd.mb,by='year')
mb.all<-rename(mb.all,c('med'='gd.med','hi'='gd.hi','lo'='gd.lo'))
mb.all<-merge(mb.all,gs.mb,by='year')
mb.all<-rename(mb.all,c('med'='gs.med','hi'='gs.hi','lo'='gs.lo'))
mb.all<-merge(mb.all,mgr.mb,by='year')
mb.all<-rename(mb.all,c('med'='mgr.med','hi'='mgr.hi','lo'='mgr.lo'))

sb.all <- merge(first.sb,last.sb,by='year')
library(plyr)
sb.all<-rename(sb.all,c('med.x'='first.med','hi.x'='first.hi','lo.x'='first.lo','med.y'='last.med','hi.y'='last.hi','lo.y'='last.lo'))
sb.all<-merge(sb.all,gd.sb,by='year')
sb.all<-rename(sb.all,c('med'='gd.med','hi'='gd.hi','lo'='gd.lo'))
sb.all<-merge(sb.all,gs.sb,by='year')
sb.all<-rename(sb.all,c('med'='gs.med','hi'='gs.hi','lo'='gs.lo'))
sb.all<-merge(sb.all,mgr.sb,by='year')
sb.all<-rename(sb.all,c('med'='mgr.med','hi'='mgr.hi','lo'='mgr.lo'))

we.all <- merge(first.we,last.we,by='year')
library(plyr)
we.all<-rename(we.all,c('med.x'='first.med','hi.x'='first.hi','lo.x'='first.lo','med.y'='last.med','hi.y'='last.hi','lo.y'='last.lo'))
we.all<-merge(we.all,gd.we,by='year')
we.all<-rename(we.all,c('med'='gd.med','hi'='gd.hi','lo'='gd.lo'))
we.all<-merge(we.all,gs.we,by='year')
we.all<-rename(we.all,c('med'='gs.med','hi'='gs.hi','lo'='gs.lo'))
we.all<-merge(we.all,mgr.we,by='year')
we.all<-rename(we.all,c('med'='mgr.med','hi'='mgr.hi','lo'='mgr.lo'))

#Subset by period, and make a table
early.df <- subset(mb.all, year %in% as.POSIXct(start_yrs))
early.mean.df <- data.frame(lapply(early.df,mean,na.rm=TRUE))

late.df  <- subset(mb.all, year %in% as.POSIXct(end_yrs))
late.mean.df <- data.frame(lapply(late.df,mean,na.rm=TRUE))
merged.df<-merge(early.mean.df,late.mean.df,all=TRUE)

#keep only those variables that use julian days
drops <- c("mgr.med","mgr.hi","mgr.lo")
jd.merged.df<-merged.df[, !names(merged.df) %in% drops]

library(tidyr)
wide.df<-gather(jd.merged.df,variable,julian.day,first.med:gs.lo,factor_key=TRUE)
#wide.df<-gather(jd.merged.df,year,variable,first.med:gs.lo)

library(ggplot2)
library(ggsignif)
theme_set(theme_bw())

texto = paste("First",cmp_yrs, "years versus last",cmp_yrs,"years")
filo = paste("compare_first_and_last_years_",cmp_yrs,".pdf")

# Plot
setwd(dirout)
pdf(file=filo, width=11, height=8.5, onefile=FALSE)
ggplot(wide.df, aes(y=variable, x=julian.day, color=factor(year))) + 
  geom_point(size=3) + 
  ...
  scale_color_discrete(name="Period",
                           labels=c("First Years", "Last Years"))+
  labs(x = "Day in Years (Julian Day)",
       y = "statistical measure & experiement",
       title="Compare Periods", 
       subtitle=texto, 
       caption="source: control run") #+ 
  #theme(axis.text.x = element_text(angle=65, vjust=0.6))
dev.off()

#Alternative plotting method
list = list(early.df = early.df, late.df = late.df)
box.merge <- ldply(list)

#keep only those variables that use julian days
drops <- c("mgr.med","mgr.hi","mgr.lo")
jd.box<-box.merge[, !names(box.merge) %in% drops]
wide.box<-gather(jd.box,variable,julian.day,first.med:gs.lo,factor_key=TRUE)

#kep only the median experiment
drops2 <- c("first.hi","first.lo","last.hi","last.lo","gd.hi","gd.lo","gs.hi","gs.lo")
temp <- jd.box[, !names(jd.box) %in% drops2]
names(temp)[names(temp) == "first.med"] <- "first.day"
names(temp)[names(temp) == "last.med"] <- "last.day"
names(temp)[names(temp) == "gs.med"] <- "growing.season"
names(temp)[names(temp) == "gd.med"] <- "growing.days"

temp <- gather(temp,variable,julian.day,first.day:growing.season,factor_key = TRUE)

setwd(dirout)
pdf(file='compare_julian_day_vars17.pdf', width=11, height=8.5, onefile=FALSE)
ggplot(temp,aes(x=variable,y=julian.day,color=.id))+geom_boxplot()+coord_flip()+
  ...
  labs(y = "Julian Day",
       ...
       title="Compare Periods", 
       subtitle=texto, 
       caption="source: 17 C run") #+ 
dev.off()

#Now same, but for mean growth rate
#keep only those variables that DO NOT use julian days

drops <- c(".id","year","mgr.med")
gr.box<-box.merge[, names(box.merge) %in% drops]
names(temp)[names(temp) == "mgr.med"] <- "mean.growth.rate"

setwd(dirout)
pdf(file='compare_growth_rate_vars17.pdf', width=11, height=8.5, onefile=FALSE)
ggplot(gr.box,aes(y=mgr.med,color=.id))+geom_boxplot()+coord_flip()+
  #geom_vline(xintercept=c(1.5,2.5,3.5,4.5))+
  scale_color_discrete(name="Period",
                       labels=c("First Years", "Last Years"))+
  labs(y = "Mean Growth Rate",
       ...
       title="Compare Periods", 
       subtitle=texto, 
       caption="source: 17 C run - all months") #+ 
dev.off()
...
# setwd(dirout)
# 
# pdf(file='maumee_5p_gs.pdf', width=11, height=8.5, onefile=FALSE)
# plot_grid(p.mb.first, p.mb.last, p.mb.mgr, p.mb.gd, p.mb.gs, 
#           labels = c("A", "B", "C", "D", "E"),
#           ncol = 3, nrow = 2)
# dev.off()
# 
# pdf(file='werie_5p_gs.pdf', width=11, height=8.5, onefile=FALSE)
# plot_grid(p.we.first, p.we.last, p.we.mgr, p.we.gd, p.we.gs, 
#           labels = c("A", "B", "C", "D", "E"),
#           ncol = 3, nrow = 2)
# dev.off()
# 
# pdf(file='sand_5p_gs.pdf', width=11, height=8.5, onefile=FALSE)
