#Owen Doherty
# Set season width from May 1 - Oct 31 for Nutrient Comparison
# Define a smaller region over which to save data; domain -83.5 to -83 and 41.6 to 41.9
# April 30, 2019- Growth Season Only, Fixed Threshold @ 17 C
...
for(i in 1:length(fnames)){
  ...
  #used a fixed threshold for all
  grt.med<- fixed.thresh
  grt.lo <- fixed.thresh
  grt.hi <- fixed.thresh
  ...
  outdir <- 'output_data/'
  ncfname <- paste(outdir,"micro-growth-werie-growsea-th17",year,".nc",sep="")
...
# create netCDF file and put arrays
outdir <- 'output_data/'
ncfname <- paste(outdir,"annual.micro.werie.growsea-th17.nc",sep="")
