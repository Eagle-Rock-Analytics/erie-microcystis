#Owen Doherty
#June 8, 2018 - Set threshold at .75% + 1.5 C
# Updated by Grace Di Cecco, 1/3/2023
...
fixed.thresh = 0.405185 #preturbation run, 19 C growth rate
...
for(i in 1:length(fnames)){
  ...
  #First day of growth
  mic.hi.first[,,i] <- apply(micro_hi,c(1,2), function(x) min(which(x>=fixed.thresh),na.rm=TRUE))
  mic.med.first[,,i] <- apply(micro_med,c(1,2), function(x) min(which(x>=fixed.thresh),na.rm=TRUE))
  mic.lo.first[,,i] <- apply(micro_lo,c(1,2), function(x) min(which(x>=fixed.thresh),na.rm=TRUE))
  ...
  #Last day of growth
  mic.lo.last[,,i] <- apply(micro_lo,c(1,2), function(x) max(which(x>=fixed.thresh),na.rm=TRUE))
  mic.med.last[,,i] <- apply(micro_med,c(1,2), function(x) max(which(x>=fixed.thresh),na.rm=TRUE))
  mic.hi.last[,,i] <- apply(micro_hi,c(1,2), function(x) max(which(x>=fixed.thresh),na.rm=TRUE))
  ...
  mic.lo.growing_days[,,i] <- apply(micro_lo,c(1,2), function(x) sum((x>=fixed.thresh),na.rm=TRUE))
  mic.med.growing_days[,,i] <- apply(micro_med,c(1,2), function(x) sum((x>=fixed.thresh),na.rm=TRUE))
  mic.hi.growing_days[,,i] <- apply(micro_hi,c(1,2), function(x) sum((x>=fixed.thresh),na.rm=TRUE))
  ...
  outdir <- 'output_data/'
  ncfname <- paste(outdir,"micro-growth-werie-th19",year,".nc",sep="")
  ncout <- nc_create(ncfname,list(lo_def,med_def,hi_def,mask_def),force_v4=F,verbose=F)
