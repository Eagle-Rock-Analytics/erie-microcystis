...
y_end = 2017
nyr = y_end-y_start+1
nlon = 370 #change this
nlat = 174 #change this
...
in_dir<-'/Volumes/data1/werie-hr/'
fstr = 'jpl-werie-hr-'
fend = '.nc'
fnames<-paste(in_dir,fstr,years,fend,sep='')
...
for(i in 1:length(fnames)){
  ...
  yyyyjjj <-paste(year,time,sep="-")
  time.stamp <- as.POSIXct(strptime(yyyyjjj,format="%Y-%j"))
  
  # UNCOMMENT THIS SECTION TO USE SEASONAL SUBSET
  # #Subset the sst data to season
  # #start day
  # sd <- as.POSIXct(strptime(paste("05-01",year,sep="-"),format="%m-%d-%Y"))
  # sdi <- format(sd,format="%j")
  # 
  # #end day
  # ed <- as.POSIXct(strptime(paste("09-30",year,sep="-"),format="%m-%d-%Y"))
  ...
  grt<- 75 #above 75% of max growth is considered possible bloom conditions
  
  micro_lo<-gr_lo[1]+(gr_lo[2]*sst.nolo)+(gr_lo[3]*(sst.nolo^2))+(gr_lo[4]*(sst.nolo^3))+(gr_lo[5]*(sst.nolo^4))
  ...
  #First day of growth
  mic-hi.first[,,i] <- apply(micro_hi,c(1,2), function(x) min(which(x>=grt),na.rm=TRUE))
  mic-med.first[,,i] <- apply(micro_med,c(1,2), function(x) min(which(x>=grt),na.rm=TRUE))
  mic-lo.first[,,i] <- apply(micro_lo,c(1,2), function(x) min(which(x>=grt),na.rm=TRUE))
  ...
  #Last day of growth
  mic-lo.last[,,i] <- apply(micro_lo,c(1,2), function(x) max(which(x>=grt),na.rm=TRUE))
  mic-med.last[,,i] <- apply(micro_med,c(1,2), function(x) max(which(x>=grt),na.rm=TRUE))
  mic-hi.last[,,i] <- apply(micro_hi,c(1,2), function(x) max(which(x>=grt),na.rm=TRUE))
  ...
  mic-lo.growing_days[,,i] <- apply(micro_lo,c(1,2), function(x) sum((x>=grt),na.rm=TRUE))
  mic-med.growing_days[,,i] <- apply(micro_med,c(1,2), function(x) sum((x>=grt),na.rm=TRUE))
  mic-hi.growing_days[,,i] <- apply(micro_hi,c(1,2), function(x) sum((x>=grt),na.rm=TRUE))
  ...
  ncfname <- paste(outdir,"micro-growth-werie-",year,".nc",sep="")
  ncout <- nc_create(ncfname,list(lo_def,med_def,mask_def),force_v4=F,verbose=F)
...
mic-med.grow.mean_def <- ncvar_def("mic-med.mean_annual_growth","% of max gr",list(londim,latdim,timedim),fillvalue,dlname,prec="double")
mic-hi.grow.mean_def <- ncvar_def("mic-hi.mean_annual_growth","% of max gr",list(londim,latdim,timedim),fillvalue,dlname,prec="double")
...
# create netCDF file and put arrays
outdir <- '/Volumes/data1/werie-hr/'
ncfname <- paste(outdir,"annual.growseason.werie.nc",sep="")
ncout <- nc_create(ncfname,list(mic-med.grow.mean_def,mic-hi.grow.mean_def,sst.mean_def,mic-med.first_def,mic-hi.first_def,mic-med.last_def,mic-hi.last_def,mic-med.season_window_def,mic-hi.season_window_def,mic-med.growing_days_def,mic-hi.growing_days_def,mask_def),force_v4=F,verbose=F)
