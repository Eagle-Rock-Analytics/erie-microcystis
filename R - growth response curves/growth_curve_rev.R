...
le_in <- read_excel("erie_results_formatted_rev4.19.xlsx")
...
#Test fittings
library(ggplot2)
ggplot(pred_4,aes(y=value.fit,x=Temperature,color=Experiment.Name))+geom_line()+ylim(-0.05,0.75)

ggplot(pred_4,aes(y=value.fit,x=Temperature,color=Experiment.Name))+geom_line()+facet_grid(.~Experiment.Name,as.table=FALSE)+ylim(-0.1,0.75)+theme(legend.position="bottom")
ggplot(pred_2,aes(y=value.fit,x=Temperature,color=Experiment.Name))+geom_line()+facet_grid(.~Experiment.Name,as.table=FALSE)+ylim(-0.1,0.75)+theme(legend.position="bottom")
...
keep2 = c("Gobler-1r","Gobler-2r","Gobler-3r","Gobler-4r","Kruger & Eloff 1978a")
...
keep4 = c("Gobler-2r","Gobler-3r","Gobler-4r","Coles & Jones 2000a","Kruger & Eloff 1978a")
