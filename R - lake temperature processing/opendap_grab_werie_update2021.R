...
#Load Example File
#Check to see if this has been updated - new version?
...
ds1.names <- names(ds1[['var']])
mask.name <- ds1$var[[3]]

mask <- ncvar_get(ds1,"analysed_sst",verbose = TRUE) #appears to have order time, lat, lon - this fails "file not found" even though extractions of other atts work
#mask <- ncvar_get(ds1,"lat") # so this works - attributes can be extracted, but not variables??

mask <- ncvar_get(ds1, mask.name,start=c(xmin,ymin,1), count=c(xcount,ycount,1),verbose=TRUE)
...
for (j in 2021) {
  ...
  for (i in 1:tcount) # 2021 missing DOY 51 & 52, no data yet beyond DOY 343: fill w/ NA
  {
    if(j == 2021 & i %in% c(51, 52, 344:365)){
      print(paste("skip day ",jd,"in year",yyyy,"counter is",i))
